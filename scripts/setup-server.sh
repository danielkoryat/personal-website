...
ExecStart=/usr/local/bin/docker-compose up -d daniel-koryat-portfolio-blue nginx certbot
ExecStop=/usr/local/bin/docker-compose down
ExecReload=/bin/bash -c '/usr/local/bin/docker-compose up -d daniel-koryat-portfolio-blue nginx certbot'
TimeoutStartSec=0
Restart=on-failure
RestartSec=30
